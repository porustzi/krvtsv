        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="min-w-0">
            <motion.div {...fadeUp(0)} className="mb-4">
              <span className="inline-block text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] text-rose-500 bg-rose-50 px-3 sm:px-4 py-2 rounded-full">
                Веб-студія / Ukraine
              </span>
            </motion.div>

            <motion.h1 {...fadeUp(0.1)} className="text-[clamp(2rem,7vw,5.5rem)] font-black leading-[0.9] tracking-tight text-gray-900 uppercase mb-4 break-words">
              САЙТИ, ЯКІ<br />
              <span className="text-rose-500">ПРОДАЮТЬ</span>
            </motion.h1>

            <motion.div {...fadeUp(0.25)} className="mt-6 mb-4">
              <p className="text-[clamp(0.875rem,3vw,1.4rem)] font-bold uppercase tracking-wide text-gray-500 break-words">
                ШВИДКО. СТИЛЬНО. НОРМАЛЬНО.
              </p>
              <p className="text-[clamp(0.875rem,3vw,1.4rem)] font-black uppercase tracking-wide text-rose-400 break-words">
                БЕЗ ПЕРЕПЛАТ
              </p>
            </motion.div>

            <motion.p {...fadeUp(0.35)} className="text-gray-500 text-base sm:text-lg leading-relaxed max-w-md mb-10 break-words">
            ...
            <motion.div {...fadeUp(0.45)} className="flex flex-wrap gap-3 sm:gap-4">
              <a
                href="#project"
                className="group inline-flex items-center gap-2 bg-gray-900 text-white font-black uppercase tracking-widest text-xs sm:text-sm px-6 sm:px-8 py-4 rounded-full hover:bg-rose-500 transition-colors duration-300 whitespace-nowrap"
              >
                ТОП ПРОЄКТ
              ...
              <a
                ...
                className="group inline-flex items-center gap-2 border-2 border-gray-900 text-gray-900 font-black uppercase tracking-widest text-xs sm:text-sm px-6 sm:px-8 py-4 rounded-full hover:bg-gray-900 hover:text-white transition-colors duration-300 whitespace-nowrap"
              >
            ...
            <motion.div {...fadeUp(0.55)} className="mt-10 sm:mt-12 flex flex-wrap items-center gap-x-6 gap-y-3">
              <div className="flex-shrink-0">
                <p className="text-2xl sm:text-3xl font-black text-gray-900">50+</p>
                <p className="text-[10px] sm:text-xs uppercase tracking-widest text-gray-400 font-semibold whitespace-nowrap">Проєктів</p>
              </div>
              <div className="w-px h-8 sm:h-10 bg-rose-200 flex-shrink-0" />
              <div className="flex-shrink-0">
                <p className="text-2xl sm:text-3xl font-black text-gray-900">3 дні</p>
                <p className="text-[10px] sm:text-xs uppercase tracking-widest text-gray-400 font-semibold whitespace-nowrap">Мін. терміни</p>
              </div>
              <div className="w-px h-8 sm:h-10 bg-rose-200 flex-shrink-0" />
              <div className="flex-shrink-0">
                <p className="text-2xl sm:text-3xl font-black text-gray-900">100%</p>
                <p className="text-[10px] sm:text-xs uppercase tracking-widest text-gray-400 font-semibold whitespace-nowrap">Задоволених</p>
              </div>
            </motion.div>
          </div>
